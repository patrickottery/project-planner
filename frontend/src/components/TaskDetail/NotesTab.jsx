          ...
          <div key={n.id} className="note-item">
            <div className="note-header">
              ...
              <div className="note-actions">
                <button className="btn-icon" onClick={() => { setEditingId(n.id); setEditBody(n.body); }}>
                  <Edit2 size={12} />
                </button>
                <button className="btn-icon danger" onClick={() => handleDelete(n.id)}>
                  <Trash2 size={12} />
                </button>
              </div>
            </div>
            {editingId === n.id ? (
              <div className="note-edit">
                <textarea rows={4} value={editBody} onChange={(e) => setEditBody(e.target.value)}
                  onKeyDown={(e) => { if (e.key === "Enter" && e.ctrlKey) handleEdit(n.id); }} />
